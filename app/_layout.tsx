...
// ─── Proactive token validation on foreground ─────────────────────────────────
    ...
    const sub = AppState.addEventListener("change", (nextState) => {
      if (
        ...
        if (useAuthStore.getState().accessToken) {
          // 401 handled globally in api.ts — expireSession() fires there.
          // On success, persist the fresh user so plan/subscription changes
          // made in Stripe (or elsewhere) are reflected immediately on return.
          api.me.get()
            .then((freshUser) => useAuthStore.getState().setUser(freshUser))
            .catch(() => {});
        }
      }
...
async function registerPushToken() {
  const { accessToken, setPushToken } = useAuthStore.getState();
  ...
  try {
    ...
    setPushToken(token);
    await api.me.update({ pushToken: token });
  } catch {
...
function PushSetup() {
  ...
  // Send device timezone on auth confirm
  useEffect(() => {
  ...
  // Register on login / hydration
  useEffect(() => {
    if (!hydrated || !accessToken) return;
    registerPushToken();
  }, [hydrated, accessToken]);

  // Re-register on every foreground transition (tokens can rotate)
  useEffect(() => {
    const sub = AppState.addEventListener("change", (nextState) => {
      if (
        appState.current.match(/inactive|background/) &&
...
function NotificationTapHandler() {
  ...
  // Taps while app is running (foreground or background)
  useEffect(() => {
