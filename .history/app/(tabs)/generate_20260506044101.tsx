      ...
      if (mode === "reel") {
        ...
        const res = await api.generate.create({
          ...
          ...(buyerTypes.size > 0
            ...
            : {}),
          ...(user?.marketLocation
            ? { market_location: user.marketLocation }
            : {}),
          ...(user?.brokerageName
      ...
      >
        ...
        <View style={styles.platformGrid}>
          {(mode === "reel"
            ...
            const disabled =
              mode === "carousel" && (p === "twitter" || p === "tiktok");
            return (
              <TouchableOpacity
                key={p}
                style={[
        ...
        {mode !== "carousel" && (
          <>
            ...
            <Text style={styles.photoSectionHint}>
              You may test a photo without generating a post. Just "Add a
              listing photo" to score your photo.
            </Text>
            {!photoUri ? (
              <TouchableOpacity
            ...
            <View style={styles.carouselGrid}>
              {Array.from({ length: Math.min(slides.length + 1, 10) }).map(
                (_, i) => {
                  ...
                  const isHook = i === 0;
                  const isCta =
                    isFilled && i === slides.length - 1 && slides.length >= 2;
                  const label = isHook
                    ? "Slide 1 — Hook"
                    : isCta
                      ? `Slide ${slideNum} — CTA`
                    ...
                    return (
                      <View key={i} style={styles.slideZone}>
                        ...
                        <View style={styles.slideLabelOverlay}>
                          <Text
                            style={[
                              styles.slideLabelText,
                              labelIsTeal && styles.slideLabelTeal,
                            ]}
                          >
                            {label}
                          </Text>
                  ...
                  return (
                    ...
                    >
                      ...
                      <Text
                        style={[
                          styles.slideEmptyLabel,
                          isHook && styles.slideEmptyLabelTeal,
                        ]}
                      >
                        {label}
                      </Text>
          ...
          {loading ? (
            <View style={styles.buttonInner}>
              ...
              <Text style={styles.generateButtonText}>
                {mode === "carousel" ? "Generating..." : "Generating..."}
              </Text>
            </View>
          ) : (
            <Text style={styles.generateButtonText}>
            ...
            {reelResult.script.tiktok_repurpose_hook && (
              <View style={styles.reelTikTokBlock}>
                ...
                <Text style={styles.reelTikTokSubtitle}>
                  Use this opening line when posting this reel to TikTok. Swap
                  it in place of your Instagram hook.
                </Text>
