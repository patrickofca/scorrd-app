...
export default function AnalyzeScreen() {
  ...
  const [image, setImage] = useState<{ base64: string; uri: string; type: string } | null>(null);
  ...
  useFocusEffect(
    useCallback(() => {
      ...
      if (prefill) {
        ...
        if (prefill.buyerTypes.length > 0) setBuyerTypes(new Set(prefill.buyerTypes));
        usePreFillStore.getState().clear();
      }

      const draft = useFormDraftStore.getState().analyzeDraft;
      if (draft && useFormDraftStore.getState().returnPath === '/(tabs)/analyze') {
  ...
  async function pickImage() {
    ...
    if (status !== 'granted') {
      Alert.alert('Permission needed', 'Allow Scorrd to access your photos to analyze listing images.');
      return;
    }
  ...
  async function handleAnalyze() {
    if (!text.trim()) {
      Alert.alert('Add your post', 'Paste or type the social media post you want to analyze.');
      return;
    }
  ...
  return (
    <SafeAreaView style={styles.safe} edges={['top']}>
      ...
      >
        ...
        <Text style={styles.subheading}>Paste your post and get an instant score</Text>
        ...
        >
          {CONTENT_TYPES.map((ct) => (
            <TouchableOpacity
              key={ct}
              style={[styles.typePill, contentType === ct && styles.typePillSelected]}
              onPress={() => {
              ...
              activeOpacity={0.7}
            >
              <Text style={[styles.typePillLabel, contentType === ct && styles.typePillLabelSelected]}>
        ...
        />
        <Text style={styles.charCount}>{text.length} / {PLATFORM_CHAR_LIMITS[platform] ?? 5000}</Text>

        {/* Image picker */}
        <Text style={styles.sectionLabel}>Listing Image <Text style={styles.optional}>(optional)</Text></Text>
        <TouchableOpacity style={styles.imageZone} onPress={pickImage} activeOpacity={0.8}>
          {image ? (
            ...
            <View style={styles.imagePlaceholder}>
              ...
              <Text style={styles.imagePlaceholderText}>Tap to add a listing photo</Text>
              <Text style={styles.imagePlaceholderSub}>Scored for staging, lighting & platform fit</Text>
            </View>
          )}
        </TouchableOpacity>
        ...
        <Text style={styles.sectionLabel}>
          Audience Targeting <Text style={styles.optional}>(optional — unlocks Match Score)</Text>
        </Text>
        ...
        {recentAnalyses.length > 0 && (
          <View style={styles.recentSection}>
            ...
            {loadingRecent ? (
              <ActivityIndicator color={Colors.teal} style={styles.recentLoader} />
            ) : (
              recentAnalyses.map((analysis) => (
                ...
                >
                  <View style={styles.recentLeft}>
                    <Text style={styles.recentPlatform}>{analysis.post?.platform ?? '—'}</Text>
                    ...
                    <View style={styles.recentBadgeRow}>
                      ...
                      {analysis.platformFit && analysis.platformFit.length > 0 && (() => {
                        const best = [...analysis.platformFit].sort((a, b) => b.fit_score - a.fit_score)[0];
                        const label = PLATFORM_LABELS[best.platform as Platform] ?? best.platform;
                        return (
                          <Text style={styles.recentBadge}>
                  ...
                  <Text style={[styles.recentScore, { color: scoreColor(analysis.compositeScore) }]}>
...
const styles = StyleSheet.create({
  ...
  imagePlaceholder: { alignItems: 'center', justifyContent: 'center', padding: 24, gap: 8 },
  ...
  recentBadgeRow: { flexDirection: 'row', flexWrap: 'wrap', gap: 6, marginBottom: 4, marginTop: 2 },
  recentBadge: {
    fontSize: FontSize.xs,
    fontFamily: FontFamily.mono,
    color: Colors.textSecondary,
